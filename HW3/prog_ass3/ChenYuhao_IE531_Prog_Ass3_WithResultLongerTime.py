...
def randomized_select_with_multipe_pivots (current_array, k, no_of_pivots) :
    ...
    if len(current_array)<no_of_pivots:
        pivot_list=np.random.randint(min(current_array)-1,max(current_array)+1,no_of_pivots)
    else:
        while i<=no_of_pivots:
           # print('min=',min(current_array),'max=',max(current_array),no_of_pivots)
            random_num=np.random.randint(min(current_array)-1,max(current_array)+1)
           # print('pivot_list=',pivot_list,'random_num=',random_num)
            if random_num not in pivot_list:
                pivot_list.extend([random_num])
                i+=1
    for j in pivot_list:
        array_splitted,type_of_pivot=split_into_three(current_array,j,k)
...
for number_of_pivots in range(1, max_no_of_pivots+1) :
    ...
    for j in range(1, 40) :
       ...
       #     print('number_of_pivots=',number_of_pivots,'j=',j)
       #     print('answer1=',answer1,'answer2=',answer2)
        ...
        #print('meaning_running_time=',mean_running_time)
        #print('std_dev_running_time',std_dev_running_time)
    #print('pivot_num=',number_of_pivots)
