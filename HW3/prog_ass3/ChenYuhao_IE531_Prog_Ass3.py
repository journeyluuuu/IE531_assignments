...
for number_of_pivots in range(1, max_no_of_pivots+1) :
    ...
    for j in range(1, 40) :
        ...
        for i in range(1, number_of_trials) :
            ...
            t3=time.time()
            pivot_list=np.random.randint(min(my_array)-1,max(my_array)+1,number_of_pivots)
            t4=time.time()
            generating_time.extend([t4-t3])
        ...
        mean_generating_time.extend([np.mean(generating_time)])
    print('ratio of random number over total=',np.mean(mean_generating_time)/np.mean(mean_running_time))
